...
def viterbi_p2(train, test):
    ...
    for sentence in train:
        for wordtag in sentence:
            ...
            if tag not in ptag:
                ...
                ptag[tag] += 1
            if word not in hapax:
                hapax[word] = (True,wordtag) 
            else:
                hapax[word] = (False,wordtag)
    
    for itm in hapax:
        bol = hapax.get(itm)
        if bol[0] == True:
            n += 1
            if bol[1][1] not in thapax:
                thapax[bol[1][1]] = 1
            else:
                thapax[bol[1][1]] += 1

    for t in thapax:
        thapax[t] = thapax.get(t) / n
    
    for x in ptag:
        if x in pinit:
            pinit[x] = np.log((pinit[x]+smol)/(len(train)+smol*len(ptag)))
        else:
            uinit[x] = np.log((smol)/(len(train)+smol*len(ptag)))
    ...
    for q in ntrans:
        fp = np.log(smol/(ptag.get(q) + smol*len(ptag)))
        utrans[q] = fp
    ...
    for p in pem:
        tag = p[1] 
        if tag in thapax:
            fp = np.log((pem.get(p)+smol*thapax.get(tag))/(ptag.get(tag)+smol*thapax.get(tag)*len(ptag)))
        else:
            fp = np.log((pem.get(p)+smol*1/n)/(ptag.get(tag)+smol*len(ptag)))
        pem[p] = fp
    
    for q in nems:
        if q in thapax:
            fp = np.log(smol*thapax.get(q)/(ptag.get(q) + smol*thapax.get(q)*len(ptag)))
        else:
            fp = np.log((smol*1/n)/(ptag.get(q)+smol*len(ptag)))
        uem[q] = fp
