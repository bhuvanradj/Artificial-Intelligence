...
def doesArmTouchObjects(armPosDist, objects, isGoal=False):
    ...
    for arm in armPosDist:
        ...
        for obj in objects: 
            ...
            if(isGoal==False):
                r = r + arm[2]
            center = np.array([obj[0],obj[1]])
            b = 2*np.dot(vec,start-center)
            c = np.dot(center,center) + np.dot(start,start) - 2*np.dot(start,center) - r**2
            d = b**2 - 4*a*c
            if(d>0 and (0<=((-b+math.sqrt(d))/(2*a))<=1 or 0<=((-b-math.sqrt(d))/(2*a))<=1)):
                return True           
    return False
...
def doesArmTipTouchGoals(armEnd, goals):
    ...
    """
    for g in goals:
        d = math.sqrt((armEnd[0]-g[0])**2 + (armEnd[1]-g[1])**2)
        if(d<=g[2]): return True
    return False
...
def isArmWithinWindow(armPos, window):
    ...
    """
    for a in armPos:
        if((a[0][0]>window[0] or a[0][0]<0) or (a[1][0]>window[0] or a[1][0]<0) or (a[0][1]>window[1] or a[0][1]<0) or (a[1][1]>window[1] or a[1][1]<0)):
            return False
    return True
