...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        """
        s = states[0]
        return [self.model[self.discretize(s)]]
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        """
        statedisc = self.discretize(state)
        statequal = self.model[statedisc][action]
        nsdisc = self.discretize(next_state)
        nsqual = max(self.model[nsdisc])
        target = 0

        if(next_state[0]>=0.5 and done):
            target = 1.0
        else:
            target = reward + self.gamma*nsqual
        uq = statequal + self.lr*(target-statequal)
        self.model[statedisc][action] = uq
        l = math.pow(statequal - target,2)
        return l
